// ------- Contralador para listar todos os produtos -----

function getProdutos(req, res){

    // Simulando os dados dos produtos do acesso ao bando de dados
    const produtos = [
        { id: 1, nome: "Produto A", preco: 50.0},
        { id: 2, nome: "Produto B", preco: 30.0},
    ];
    // Definindo o status de sucesso e envia a lista de produtos como JSON
...
function createProduto(req, res){
 ...
 // Recebe os dados do corpo da requisição em partes (chunks)
    ...
    // Recebendo os dados após a recepção completa
    ...
    req.on("end", () => {
        try{
            ...
            res.statusCode = 201;
        res.end(JSON.stringify({message:"Produto criado com sucesso!", produto : novoProduto}));
        }catch{
...
function updateProduto(req, res){
    const id = req.url.split("/")[3] // Extrai a ID da URL)
    let body = "";
    ...
    req.on("end", () =>{
        try{
            ...
            res.statusCode = 200;
            res.end(JSON.stringify({message: "Produto atualizado com sucesso!", produto: produtoAtualizado}));
  ...
  function deleteProduto(req, res){
    const id = req.url.split("/")[3] // Exrai o ID da Url 

      // Define o status de sucesso e envia uma mensagem confirmando a exclusão
    
    res.statusCode = 200;
    res.end(JSON.stringify({message: `Produto com o ${id} deletado`}));
}
